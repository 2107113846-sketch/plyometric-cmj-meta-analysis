...
add_para(doc,
    ...
    '并作为敏感性分析单独处理，臂位未明6篇，CMJA带臂9篇，VJ带臂1篇。',
    size=Pt(12), first_line_indent=Cm(0.74))
...
add_para(doc,
    ...
    '其中严格手叉腰无臂CMJ 16篇，臂位未明6篇，CMJA带臂6篇，VJ带臂敏感性亚组1篇。')
...
add_para(doc,
    '降级理由：¹PEDro中位5.9/10，分配隐藏仅7%，评估者盲法仅15%；²主分析池I²=68-78%，'
...
add_para(doc,
    ...
    'CMJA带臂（k=6）：g=+0.98[0.21,1.74], I²=79%。臂位未明组I²最低（20%），效应量中等偏大——这是一个值得关注的模式：严格手叉腰组异质性（I²=78%）反而高于臂位未明组（I²=20%），与"手臂位置分层排除噪声"的预期方向相反。'
...
add_para(doc,
    ...
    '本研究的合并SMD（g=+1.11）在原始量尺上约对应CMJ增加3-5 cm（基于从纳入研究计算的'
    '从纳入研究的基线CMJ SD计算得到的加权合并基线SD≈4.5 cm（基于16篇研究报告基线标准差，I²=98.7%，反映跨研究SD的巨大异质性——纳入研究SD范围为0.8-16cm——该值仅为示意性近似）作为示意性近似值，具体映射因人群基线而异），'
