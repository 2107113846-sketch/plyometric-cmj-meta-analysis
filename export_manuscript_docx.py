...
add_para(doc,
    ...
    'I²=76.2%[95%CI:54.6%,87.3%]，预测区间[-0.878,+3.134]。宽版池g=+1.002[95%CI:+0.722,+1.282]，I²=66.9%[95%CI:45.7%,80.1%]。'
...
add_para(doc,
    ...
    '预测区间：[-0.878, +3.134]；异质性：τ²=0.687，I²=76.2% [95% CI: 54.6%, 87.3%]，Q(15)=52.30，p<0.001。')
add_para(doc,
    '宽版全池（28篇RCT）：合并Hedges\' g = +1.002 [95% CI: +0.722, +1.282]，p<0.001；'
    '异质性：τ²=0.359，I²=66.9% [95% CI: 45.7%, 80.1%]，Q(27)=77.23，p<0.001。')
add_para(doc,
...
add_para(doc,
    '短期Plyo训练的合并效应最为可靠：I²仅13% [95% CI: 0%, 55%]，95%CI紧凑，效应属中等偏大（g≈0.60）。'
    '干预时长是解释异质性的首要调节变量。')
...
add_para(doc,
    ...
    '的发现一致。校正后的效应量仍属大效应范畴（g>0.8），进一步支持了Plyometric训练对CMJ高度'
    '的正向效应。')
...
add_para(doc,
    '降级理由：¹PEDro中位5.9/10，分配隐藏仅14%，评估者盲法仅17%；²主分析池I²=67-76%，'
    '属严重不一致；³干预/结局/人群直接对应PICOS；⁴多项研究n<20，预测区间跨零；'
    '⁵Egger p<0.001，SE-g r=0.88，信号强烈；⁶I²=13%；⁷k=2时I²=0%解读需谨慎；'
    '⁸k=2，CI宽。',
    size=Pt(10), first_line_indent=Cm(0.74), spacing_after=Pt(4))
add_para(doc,
    'GRADE总结：Plyometric训练对CMJ高度的总体证据确定性为低(Low)，主要受高异质性和发表偏倚'
    '信号影响。短期(≤6周)干预的证据确定性为中等(Moderate, I²=13%)，是本研究中最可靠的效应量'
    '估计。青少年亚组证据等级为中等，但研究数量有限（各2篇）。长期(>10周)干预因k=4且CI跨零，'
    '证据等级为极低(Very Low)。')
...
add_para(doc,
    ...
    '95%置信区间跨越零值。相比之下，短期亚组（≤6周，k=14）虽然效应量中等（g=+0.60），但I²仅13% [95% CI: 0%, 55%]，是本研究'
...
coach_recs = [
    ...
    '（g≈+0.60，原始量尺约对应CMJ增加2.4-3.6cm），且效果一致性高（I²=13%），建议作为团队运动季前'
