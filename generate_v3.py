...
add_para(doc,
    ...
    '（1）数据分析代码的生成与调试（R/metafor及Python脚本）；（2）初稿文本的格式化组织与语法校对。'
...
add_para(doc,
    '初步检索获得文献135篇。排除重复及标题/摘要筛选44篇（其中重复27篇、无关主题17篇）后，对91篇进行全文评估。排除62篇'
...
add_para(doc,
    ...
    '（结果见敏感性分析）。\n\n'
    '值得注意的是，Egger回归截距为负值（−2.64），提示大样本研究效应偏大；但SE-g相关为强正值（r=+0.88），'
    '传统上被解读为小样本研究效应偏大。这一方向矛盾可能与高异质性（I²=78.1%）条件下两种方法对小研究效应的不同敏感性有关——'
    'Egger回归检验的是效应量精度的线性函数（截距为负表示随精度增加效应量减小，即小样本效应大），'
    '而SE-g rank相关直接检验效应量与标准误的排序关系。在高异质性条件下，效应量与标准误的关系可能因异质性来源（如训练方案规范性、基线体能水平等）的结构化分布'
    '而呈现复杂的非单调模式，两种方法可能捕获了该模式的不同维度。因此，发表偏倚的单一方法评估结果应谨慎解读，并建议辅以'
...
limitations = [
    ...
    ('数据估算。',
     ...
     '因此这些估算对总体结论无实质影响。此外，r=0.7假设（前测-后测相关系数）基于CMJ力台重测信度文献'
     '（Claudino等2017），但敏感性分析显示该假设对合并效应量有实质影响：r从0.5'
    ...
    ('缺乏PEDro分层敏感性分析。',
     '本研究未报告按PEDro评分排除低质量研究（如PEDro<5）后的敏感性分析结果，无法评估研究质量对合并效应量的定量影响。'),
    ('剂量-反应分析方法局限。',
     '本研究仅采用线性Meta回归和分类亚组分析评估剂量-反应关系。未来研究可采用限制性立方样条（Restricted Cubic Splines）等非线性方法更灵活地建模效应量随干预时长的变化曲线。'),
]
