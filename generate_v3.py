...
add_para(doc,
    ...
    f'干预时长是最重要的调节变量：短期(≤6周)g={fmt_g(short_g)}(I²={fmt_pct(short_I2)})、'
    f'中期(7-10周)g={fmt_g(mid_g)}(I²={fmt_pct(mid_I2)})、'
    f'长期(>10周)g=+1.85(I²=94%)。Meta回归确认每周SMD增加约0.13'
    f'（b=+0.131, SE=0.058, z=2.26, p=0.023）。青少年亚组效应最大(g=+1.37~1.51,I²=0%)。'
    f'Egger检验显著(p<0.001)，SE-g高度相关(r=+0.88)。Trim-and-Fill校正后效应量反增（严格池+13.8%），提示不对称性非经典发表偏倚。'
...
add_para(doc, '解读：', bold=True, size=Pt(12), first_line_indent=Cm(0.74),
         spacing_after=Pt(2))
add_para(doc,
    'Plyometric训练对CMJ高度有显著的大效应提升，短期训练(≤6周)即可获得可靠的中等效应。'
    '干预时长与效应量呈非线性剂量-反应关系（短期g≈+0.71→中期g≈+0.95，增幅平缓；中期→长期g≈+1.85，出现显著跃升）。'
...
intro_paragraphs = [
    ...
    '这一机制最早由Bosco等（1982）通过人体跳跃实验系统证实[10]。',
...
add_table(doc,
    ...
    [
        ...
        ['中期7-10周', str(mid_k), fmt_g(round(mid_g,2)) + '[' + fmt_ci_pair(mid_cl, mid_cu).strip('[]') + ']',
         '⬇严重¹', '⬇严重²', '✅无降级³', '⬇严重⁴', '⬇严重⁵', '低(Low)'],
...
add_para(doc,
    ...
    '⁵Egger p<0.001，SE-g r=0.88，信号强烈；⁶I²=' + fmt_pct(short_I2) + '[0%,' + fmt_pct(short_I2_u) + ']；⁷k=2，I²=0%但95%CI不可计算（k<3），'
...
findings = [
    f'Plyometric训练对CMJ高度有显著的大效应提升：严格手叉腰池合并Hedges\' g={fmt_g(round(strict_g,2))}[95%CI:{fmt_ci_pair(strict_cl, strict_cu).strip("[]")}，'
    f'宽版全池g=+0.99[95%CI:+0.70,+1.28]，效应量均属Cohen\'s标准下的大效应范围（g>0.8）。',
    ...
    f'（>6周）效应量变化呈现非线性模式（短期→中期g=+0.71→+{fmt_g(round(mid_g,2))}, I²={fmt_pct(mid_I2)}；'
    f'中期→长期g=+{fmt_g(round(mid_g,2))}→+1.85, I²=94%）。Meta回归确认了显著的线性时长-效应关系'
...
add_para(doc,
    f'然而，需要审慎解读的是，亚组分析揭示的剂量-反应关系并非简单的线性递增，而是呈现出明显的非线性特征。'
    f'短期亚组（≤6周，k={short_k}）效应量中等（g≈{fmt_g(short_g)}），异质性低（I²={fmt_pct(short_I2)}），是本研究中最可靠的估计。'
...
add_para(doc,
    ...
    'CMJA带臂（k=6）：g=+0.98[0.21,1.74], I²=79%。臂位未明组I²最低（20%），效应量中等偏大——这是一个值得关注的模式：'
...
add_para(doc,
    ...
    'CMJ高度的MCID在运动科学实践中通常估计约为1.5-3.0 cm（基于Claudino等2017年Meta分析[4]中报告的CMJ典型测量误差和最小可检测变化）。'
...
coach_recs = [
    ...
    '[Meta分析证据：青春期前/青春期亚组各k=2, I²=0%但证据不足；训练安全性建议基于外部文献[11,12]]',
]
...
references = [
    ...
    '[28] Byrne PJ, Moran K, Rankin P, Kinsella S. Effect of a six-week plyometric training '
    ...
    '[41] Rensing N, Bartsch J, Rembitzki I, et al. Trainingseffekte eines reaktiven '
